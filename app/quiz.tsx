    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-purple-400 to-blue-300">
      ...
      {quizStarted && !showResults && questions.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md flex flex-col items-center">
          <h2 className="text-2xl font-bold mb-4" style={{color: 'var(--zevy-primary, #a855f7)'}}>Question {currentQuestion + 1} of {questions.length}</h2>
          ...
          <div className="flex flex-col gap-3 w-full">
            {questions[currentQuestion].options.map((opt: string, idx: number) => (
              <button
                key={idx}
                className="text-white py-2 px-4 rounded font-semibold transition"
                style={{background: 'var(--zevy-primary, #a855f7)'}}
      ...
      {showResults && (
        <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md flex flex-col items-center">
          <h2 className="text-2xl font-bold mb-4" style={{color: 'var(--zevy-primary, #a855f7)'}}>Quiz Results</h2>
          <p className="mb-2 text-lg">Score: {getScore()} / {questions.length}</p>
          <button
            className="mt-4 text-white font-bold py-2 px-4 rounded transition"
            style={{background: 'var(--zevy-primary, #a855f7)'}}
